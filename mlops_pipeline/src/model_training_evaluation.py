...
def train_and_evaluate_models(X_train, y_train, X_test, y_test, model_names):
    ...
    for name in model_names:
        ...
        if isinstance(model, GridSearchCV):
            ...
            print(f"Mejores parametros para {name}: {best_params}")
...
def save_results(results_df, path="artifacts/model_results.csv"):
    """
    Guarda tabla resumen de metricas.
    """
...
def plot_model_comparison(results_df, save_path="artifacts/model_comparison.png"):
    """
    Genera grafico comparativo de metricas entre modelos.
    Incluye metricas generales y especificas para la clase minoritaria.
    """
    metrics_cols = [
        "accuracy",
        "precision_weighted",
...
def print_final_report(model, X_test, y_test):
    """
    Imprime matriz de confusion y reporte de clasificacion detallado.
    """
    y_pred = model.predict(X_test)
    print("\nMatriz de Confusion:")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print_final_report(best_model, X_test, y_test)
    print("=" * 70)
