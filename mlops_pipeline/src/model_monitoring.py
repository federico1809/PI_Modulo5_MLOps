...
def select_monitoring_features(
    ...
    if only_baseline:
        logger.warning(f"Columnas solo en baseline (ignoradas): {only_baseline}")
    if only_current:
        logger.warning(f"Columnas solo en current (ignoradas): {only_current}")
...
def psi_drift(
    ...
    current_clean = current.dropna()
    if len(baseline_clean) < min_sample_size:
        return np.nan, f"Baseline tiene solo {len(baseline_clean)} muestras"
    if len(current_clean) < min_sample_size:
        return np.nan, f"Current tiene solo {len(current_clean)} muestras"
    if baseline_clean.empty or current_clean.empty:
        return np.nan, EMPTY_DATA_MSG
    try:
    ...
    if len(breakpoints) <= 2:
        return np.nan, "Muy pocos bins únicos para calcular PSI"
...
def compute_drift_metrics(
    ...
    numeric_features, categorical_features = identify_feature_types(baseline_df)
    results = []
...
if __name__ == "__main__":
    ...
    #
    # FIX: cutoff_date se lee desde artifacts['split_config']['cutoff_date_train']
    # generado por run_ft_engineering(), garantizando que baseline == train
    # y current == val+test, consistente con el split del pipeline.
    # Reemplaza el valor hardcodeado "2025-06-30" que podía no coincidir
    # con el corte real y dejar una de las dos poblaciones vacía.
    from ft_engineering import run_ft_engineering, build_monitoring_dataset
    _, _, _, _, _, _, artifacts = run_ft_engineering()
    cutoff_date = artifacts["split_config"]["cutoff_date_train"]
    build_monitoring_dataset(
        output_path="./Base_de_datos_monitoring.csv",
        target_col="Pago_atiempo",
        keep_date=True
    )
    logger.info(f"Usando cutoff_date desde ft_engineering: {cutoff_date}")
